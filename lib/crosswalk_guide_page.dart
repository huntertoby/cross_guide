...
class _CrosswalkGuidePageState extends State<CrosswalkGuidePage> {
  // ===== 導引／語音 =====
  bool _guiding = false;       // 決定是否渲染 YOLOView（釋放相機關鍵）
  String _hint = '初始化中…';
  Timer? _logTicker;

  // ===== 幾何與濾波狀態（純文字輸出） =====
  double _angleVertRaw = 0.0;
  ...
  // 正式前往紅綠燈頁：先移除 YOLOView（釋放相機），再 pushReplacement
  Future<void> _goToTraffic() async {
    if (_navigating) return;
    _navigating = true;

    // 1) 停導引，移除 YOLOView -> 觸發其 dispose 釋放相機
    _guiding = false;
    _logTicker?.cancel();
    _logTicker = null;
    try { TTS.i.stop(); } catch (_) {}
    ...
    // 2) 等一個 frame/短延遲，確保相機釋放完成
    await Future.delayed(const Duration(milliseconds: 120));
    if (!mounted) return;

    // 3) 以置換方式進入紅綠燈頁，避免在舊頁再度 build
  ...
  Future<void> _onResult(dynamic result) async {
    ...
    if (dets.isEmpty) { _stableCount = 0; return; }

    // 取最高信心的實例
    dynamic best = dets.first;
    double bestScore = (best.confidence ?? best.conf ?? best.score ?? 0.0).toDouble();
    for (final d in dets) {
    ...
    if (mask == null) { _stableCount = 0; return; }

    final cv.Mat m = _toMaskMat(mask);
    if (m.isEmpty) { _stableCount = 0; return; }
    ...
    if (cutOrWeak) {
      ...
      final double dx = centerX - ep.cx;  // >0：遮罩重心在左
      ...
      if (centered) {
        ...
        if (!_navigating && _stableCount >= guideConfig.requiredStable) {
          if (mounted) setState(() => _hint = '對準成功，準備辨識紅綠燈…');
      ...
      setState(() {
        _hint =
        '畫面 ${_lastFrameSize.width.toInt()}×${_lastFrameSize.height.toInt()}｜'
            '遮罩品質不足：'
            '${ep.clippedLeft ? "左貼邊 " : ""}'
            '${ep.clippedRight ? "右貼邊 " : ""}'
            '雙側列比例=${(ep.dualRowsRatio * 100).toStringAsFixed(0)}%｜'
            '穩定計數=${_stableCount}/${guideConfig.requiredStable}｜'
            '\n建議：$tip';
      });
      return;
    }

    if (ep.left.length < 10 && ep.right.length < 10) { _stableCount = 0; return; }
    ...
    final angles = <double>[];
    if (leftLine  != null) { final seg = _intersectWithTopBottom(leftLine,  _lastFrameSize); angles.add(_lineVerticalAngleDeg(seg.$1, seg.$2)); }
    if (rightLine != null) { final seg = _intersectWithTopBottom(rightLine, _lastFrameSize); angles.add(_lineVerticalAngleDeg(seg.$1, seg.$2)); }
    if (angles.isEmpty) { _stableCount = 0; return; }

    double avg = angles.reduce((a, b) => a + b) / angles.length;
    if (guideConfig.flipLR) avg = -avg;
    _angleVertRaw = avg;

    if (!_emaInited) { _angleVertEma = _angleVertRaw; _emaInited = true; }
    else { _angleVertEma = (1.0 - guideConfig.emaAlpha) * _angleVertEma + guideConfig.emaAlpha * _angleVertRaw; }

    final now = DateTime.now().millisecondsSinceEpoch;
    final double ang = _angleVertEma;
    String tip;

    if (ang >= guideConfig.deadband) {
      tip = '往右移動置中';
      _stableCount = 0;
      if (now - _lastSayMs > guideConfig.sayCooldownMs) { TTS.i.speak(tip); _lastSayMs = now; }
    } else if (ang <= -guideConfig.deadband) {
      tip = '往左移動置中';
      _stableCount = 0;
      if (now - _lastSayMs > guideConfig.sayCooldownMs) { TTS.i.speak(tip); _lastSayMs = now; }
    } else {
      final bool facingOk = ang.abs() <= guideConfig.facingBand;
      if (facingOk) { _stableCount++; tip = '對準中…'; }
      else { _stableCount = 0; tip = '保持方向'; }
      if (now - _lastSayMs > guideConfig.sayCooldownMs) { TTS.i.speak(tip); _lastSayMs = now; }
    }
    ...
    setState(() {
      _hint =
      '畫面 ${_lastFrameSize.width.toInt()}×${_lastFrameSize.height.toInt()}｜'
          '垂直角(測量/EMA)：${_angleVertEma.toStringAsFixed(1)}°（左正右負）｜'
          '穩定計數=${_stableCount}/${guideConfig.requiredStable}｜'
          '\n建議：$tip';
    });
  }

  // === Utils: 將 mask 轉 Mat(8U) ===
  cv.Mat _toMaskMat(dynamic mask) {
    ...
    if (mask is cv.Mat) {
      if (mask.type == cv.MatType.CV_8UC1) {
        final (double _, cv.Mat bin0) = cv.threshold(mask, 0, 255, cv.THRESH_BINARY | cv.THRESH_OTSU);
        return bin0;
      }
      final cv.Mat gray = cv.cvtColor(mask, cv.COLOR_BGR2GRAY);
      final (double __, cv.Mat bin) = cv.threshold(gray, 0, 255, cv.THRESH_BINARY | cv.THRESH_OTSU);
      return bin;
    }
    return cv.Mat.empty();
  }

  // === Utils: 從遮罩掃描左右邊緣點（逐列） + 品質偵測 ===
  _EdgePoints _extractEdgePointsFromMask(cv.Mat bin) {
    ...
    int totalRows = 0, dualRows = 0, hitLeftBorderRows = 0, hitRightBorderRows = 0;
    int sumX = 0, cntX = 0;
    ...
    for (int y = yStart; y < yEnd; y += guideConfig.rowStep) {
      ...
      int lx = -1, rx = -1;
      for (int x = 0; x < W; x++) {
        if (data[rowBase + x] > 0) { lx = x; break; }
      }
      if (lx == 0) hitLeftBorderRows++;

      for (int x = W - 1; x >= 0; x--) {
        if (data[rowBase + x] > 0) { rx = x; break; }
      }
    ...
    final double dualRowsRatio = (totalRows > 0) ? (dualRows / totalRows) : 0.0;
    spans.sort();
  ...
  (double, double, double, double) _fitLine(List<_P> pts) {
    ...
    final double tmp = math.sqrt((sxx - syy) * (sxx - syy) + 4 * sxy * sxy);
    final double l1 = (sxx + syy + tmp) / 2.0;
    double vx = sxy, vy = l1 - sxx;
    final double norm = math.sqrt(vx * vx + vy * vy);
    if (norm > 1e-6) { vx /= norm; vy /= norm; } else { vx = 0.0; vy = 1.0; }
    return (vx, vy, mx, my);
  }

  (Offset, Offset) _intersectWithTopBottom((double, double, double, double) line, Size s) {
    ...
    if (vy.abs() < 1e-6) {
      ...
      return (Offset(x0 + tL * vx, y0 + tL * vy), Offset(x0 + tR * vx, y0 + tR * vy));
    } else {
      final double tTop = (0 - y0) / vy;
      final double tBot = (H - y0) / vy;
      return (Offset(x0 + tTop * vx, 0), Offset(x0 + tBot * vx, H));
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Stack(
        fit: StackFit.expand,
        children: [
          // 關鍵：只有在 _guiding == true 時才渲染 YOLOView（釋放相機）
          if (_guiding)
            YOLOView(
          ...
          //         child: Text(_hint, style: const TextStyle(color: Colors.white, fontSize: 14)),
          //       ),
          //     ),
          //   ),
        ],
...
// ====== 小型結構 ======
class _P { final double x, y; const _P(this.x, this.y); }
